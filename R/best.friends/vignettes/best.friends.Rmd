...
```{r}
best.friends.bic(relation,prior.to.have.friends = .5)
```

```{r}
best.friends.bic(relation,prior.to.have.friends = .1)
```


Collection 2 is the best friend of tag 3, and it is the only result with p-value less than 0.05. Makes sense. Tag 3 is the best for the collection 2, while it is the worst for other two collections.
...
```{r}
best.friends.bic(regulation,prior.to.have.friends = .5,best.no = 1)
```

```{r}
best.friends.bic(regulation,prior.to.have.friends = .1,best.no = 1)
```
