...
```
Cloud 2 is the best friend of tag 3, and it is the only result with p-value less than 0.05. Makes sense. Tag 3 is the best in the collection 2, while it is the worst in other two clouds.
...
```
Tags (genes) are rows, collections (TFs) are columns, it is consistent with $A\left(e,c\right)$ notation.
