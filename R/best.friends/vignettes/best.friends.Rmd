...
Suppose we have set of tags $T$ and a set of clouds $C$, and each cloud pays more or less atention to each tag and there is a numeric measure of how a cloud is attentive to the tag $\mu\left(t,c\right)$ for each pair $(t,c): t \in T, c \in C$. The higher the y value is, the more the tag $t$ is of value for the cloud $c$. The absence of the tag in the cloud is shown by the value that is minimal for the cloud.

Here are some examples of what the tag and what the cloud could be.

        Example                           Tag $t$         Cloud $c$                           $A\left(t,c\right)$
--------------------------------------   -------------   ----------------------------------  ----------------------------
