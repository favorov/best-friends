...
For each entity $\epsilon \in E$ let's rank all the $A\left(f_i,\epsilon \right), f_i \in F$. Let's normalise the ranks, dividing by $\left|F\right|$, and let's denote the resulting value as $\left<f_i,\epsilon \right>$. All these normalised rank values (let's refer to them as) are in $\left[0,1\right]$. Let's look at the ranks of one selected feature $\varphi$ in different entities, $\left< \varphi, e_j \right>, e_j \in E$. If all the relations are at random (null hypothesis), the values $\left< \varphi, e_j \right>$ are independently and uniformly distributed in  $\left[0,1\right]$.  If an entity $e_j$ is friendly for the feature $\varphi$, the corresponding $\left< \varphi, e_j  \right>$ is to be lager than the values for other entitites $\left< \varphi, e_m  \right>, m \ne j$, and the difference is to be unexpectedly large for the null hypothesis. A friend of Augustus cares about Augustus more than about other persons. Statistics allows us to estimate the p-value of the difference and, if it is low, to conclude that the entity $e_j$ is the best friend of the feature $\varphi$. In this case, $\varphi$ is a marker for $e_j$. 

But, actually, the top of the ranking can be filles with $b$'s that do not specifically relate to the selected $a$, they just strongly interact with all or almost all the $a$'s. Sometimes, it makes sense (if we wnat to know what are strongest regulators for gene G?), but sometimes, we want to answer other question: what is the strongest specific regulator for gene G (let's call it the best friend of G). In our notation, $A$ is the set of genes, and $B$ is the set of regulators, and $ \left<a,b\right> $ is some regulatory strenght.
