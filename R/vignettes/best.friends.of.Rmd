...
Suppose we have set of entities $E$ and a set of features $F$, and there is a numeric measure of how a feature relates to an entity $\left<f,e\right>$ for each pair $(f,e) : f \in F, e \in E$. Let's say the higher the $\left<f,e\right>$ number is, the more $f$ relates to $e$. If ets $E$ and $F$ are the same, $\left<f,e\right>$ describes the relation between the elements of the same set  $(f,e) : f \in F, e \in F$, and the friendship terminology emerges naturally from this symmetric case. Still, the friendship relation itself is asymmetric: a friend cares about Augustus, while Augustus does not. Let's start with asymetric setup, $F \ne E$.

For each selected entity $e \in E$ we rank of all the features $f \in F$ by the relation $\left<f,e\right>$. But, actually, the top of the ranking can be filles with $b$'s that do not specifically relate to the selected $a$, they just strongly interact with all or almost all the $a$'s. Sometimes, it makes sense (if we wnat to know what are strongest regulators for gene G?), but sometimes, we want to answer other question: what is the strongest specific regulator for gene G (let's call it the best friend of G). In our notation, $A$ is the set of genes, and $B$ is the set of regulators, and $ \left<a,b\right> $ is some regulatory strenght.
