...
## Best friends
First, For every community $\varsigma, \varsigma \in C$ let's rank all the elements $e$ by the $A\left(e, \varsigma \right)$ value. The ranking is acsending, so rank 1 is for the maximal value. Let's map the ranks to  $\left[0,1\right]$ and let's denote the resulting values as $r\left(e,\varsigma \right)$ and refer to them as to ranks. Second, for every element $\epsilon$ let's order all its ranks $r \left( \epsilon , c \right)$ in different communities. The ordered values $r\left(e,c_i \right ),1 \le i \le |C|$ increase with $i$ for each $e$, the first is the best. Obviuosly, if the element $\epsilon$ has a best friend among the communities, it is $\varsigma=mainformlaheretomorrow$ If all the relations are at random (null hypothesis), the values $r\left( \varphi, e \right)$ are independently and uniformly distributed in  $\left[0,1\right]$. If an entity $e$ is friendly for the feature $\varphi$, the corresponding $r\left( \varphi, e  \right)$ is to be larger than the values for all other entitites $r\left( \varphi, e_m  \right), m \ne j$, and the difference is to be unexpectedly large for the null hypothesis. A friend of Augustus cares about Augustus more than about other persons. Let's take the difference between the mininal and the minimal of the remainder ranks (the best-after-the best) as the observation for feature $\varphi$. It characterise the differnce between the best (minimal) rank and all the others. U-statistics allows us to estimate the p-value for the observation, if it is low, to conclude that the entity $e$ is the best friend of the feature $\varphi$. In this case, $\varphi$ is a marker for $e$. For each feature, the `best.friends.of.features`, returns the possible best friend, and the p-value for the observation provided the null model. 
