...
A good [example](#toyTF) that is easy to think about is: genes are elements and their regulation by transcription factors (TF's) form communities, and $A$ shows the strength of the regulation. We look at a TF and we want to know whether a gene is its friend, i.e. whether the gene is specifically sensitive to the regulation by the TF. The na&iuml;ve idea is to look for a gene that is the most sensitive for the TF. Still, it's possible that this gene strongly interact with all the TFs. Sometimes, it is what we want to find, but now we want to answer another question: what gene G is the most specific regulation downstream for the TF and is the specificity enough to say that G is the best friend for the TF?

Sometimes, the commiuities emerges from the same set of elements, e.g. the $c_i$ is a set of neighbours of $e_i$ in some graph. In this case we can represent $A\left(c_i,e_j\right)$ as $A\left(e_i,e_j\right)$. In the example, $A$ is a weighted adjacency matrix of this graph. The friendship terminology emerges naturally from this case. The friendship relation itself is asymmetric: a friend cares about Augustus, while Augustus does not.
