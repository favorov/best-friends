...
```{r}
correl=matrix(
	c(1.00, 0.95, 0.92, 0.91, 0.97,
		0.95, 1.00, 0.75, 0.20, 0.25,
		0.92, 0.75, 1.00, 0.20, 0.22,
		0.91, 0.20, 0.20, 1.00, 0.15,
...
```{r}
genelists<-OrderByBackwardsRank(correl)
genelists
```
...
To apply most standard clustering techniques, we need a symmetric distance with self-distance = 0. It can be done if we add the direct and reverse ranks form previous function (A for B and B for A), decrease them both by 1 to achieve |GG|=0 for any gene G and, eventually, map it on [0,1] interval. We can also treat this distance as a kind of p-value is we suppose that the rank is distributed uniformly.  
