...
.BackwardsRank.data.table<-function(correlations,by.column=TRUE,similarity.measure=TRUE){
	...
	#t(sort.by.backwards.rank)	
	if(by.column){return(backwards.rank.table)}
	#if we are here, we need to transpose the thing
	backwards.rank.table.t<-data.table(gene.names)
	setnames(backwards.table.t,1,'name')
	#browser()
	for (i in 1:length(gene.names))
		backwards.rank.table.t[,eval(as.name(i)):=unlist(backwards.rank.table[i,])]
	backwards.rank.table.t
}
