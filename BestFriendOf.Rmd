...
And now, let's prepare a simple example. We have five genes (A,B,C,D,E) and three TFs (TF1,TF2,TF3). The strength of tregulation is shown by the matrix. 
...
TF.names<-c('TF1','TF2','TF3')
colnames(regulation)<-gene.names
rownames(regulation)<-TF.names
regulation
```
...
```{r}
ranklists<-RankByBestFriend(regulation)
ranklists
```
#### Best Friends ordering

orderlists<-OrderByBackwardsRank(regulation)
orderlists
```
