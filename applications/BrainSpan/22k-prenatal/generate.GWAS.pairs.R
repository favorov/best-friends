...
sink('GWAS.RBR.5.filtered.pairs.txt')
for (gwas.gene in gwas.genes) {
  gwas.gene.no<-which(gwas.gene==gene.symbols)
...
sink('GWAS.RBR.1.pairs.txt')
for (gwas.gene in gwas.genes) {
  gwas.gene.no<-which(gwas.gene==gene.symbols)
  friends<-gene.symbols[order.22k.prenatal[,gwas.gene.no][1:list.1.length]]
  gwas.genes.by.me<-setdiff(gwas.genes,c(gwas.gene))
  my.gwas.friends<-gwas.genes.by.me[gwas.genes.by.me %in% friends]
  cat(paste(gwas.gene,my.gwas.friends,sep=' -> '),sep='; ')
	cat('; ')
...
sink('GWAS.RBR.1.filtered.pairs.txt')
for (gwas.gene in gwas.genes) {
  gwas.gene.no<-which(gwas.gene==gene.symbols)
  friends<-gene.symbols[order.22k.prenatal.filtered[,gwas.gene.no][1:list.1.length]]
  gwas.genes.by.me<-setdiff(gwas.genes,c(gwas.gene))
  my.gwas.friends<-gwas.genes.by.me[gwas.genes.by.me %in% friends]
  cat(paste(gwas.gene,my.gwas.friends,sep=' -> '),sep='; ')
	cat('; ')
...
sink('GWAS.naive.1.pairs.txt')
for (gwas.gene in gwas.genes) {
  gwas.gene.no<-which(gwas.gene==gene.symbols)
  friends<-gene.symbols[order(corr.22k.prenatal[gwas.gene.no,],decreasing=TRUE)[1:list.1.length]]
  gwas.genes.by.me<-setdiff(gwas.genes,c(gwas.gene))
  my.gwas.friends<-gwas.genes.by.me[gwas.genes.by.me %in% friends]
  cat(paste(gwas.gene,my.gwas.friends,sep=' -> '),sep='; ')
	cat('; ')
